...
<p>The possible combinations of 5 letters, chosen with replacement, from the pool of 26 is <code>26^5</code> which is a lot, but not insanely many. I work with multi-million row datasets frequently enough. So, we could just run a loop over all integers (<code>set.seed</code> rounds to nearest integer anyway; refer to <code>?set.seed</code>), set the seed to that value, and save the sampled letters. The first combination will be</p>
